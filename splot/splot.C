...
//Here the data consists of signal events and background events. 
// The method describes how to separate signal events from background.
//So two variables are used , one is discriminating variable and another one is 
// the control varible. Here discriminating variable is jpsi mass and the control 
// variable is energy/momentum. At first, We generate a two component data set from 
// two varibles stored in the ntuple. Then fit the jpsi mass with two pdfs. Then calculate 
// the signal and background weights to plot E/p splots. Two splots are found with 
// signal contributions and backgrounds.
