    ...
    for iEvent in range(total_event_number[root_file_no]):
        ...
        if (variable_name == "Mbc"):
            if (
                ...
                histogram[root_file_no].Fill(value_of_var)
        elif (variable_name == "deltaE"):
            if (
                (value_of_var > lower_range and value_of_var < upper_range) and
...
else: legend = ROOT.TLegend(0.1,0.45,0.3,0.65)
c1.cd()
...
c1.Update()
c1.SaveAs(f"plot_from_my_anyfile_anyvariable_bin_range/{output_filename}_kin_veto_sigprob21.png")